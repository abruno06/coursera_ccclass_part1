...
public class AirportAirportDeparturePerformance extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		jobTop.setMapperClass(TopAirportAirportMap.class);
	//	jobTop.setReducerClass(TopAirportCarrierReduce.class);
		jobTop.setReducerClass(TopAirportAirportReduceToCassandra.class);
	...
	//	jobTop.setOutputFormatClass(TextOutputFormat.class);
		...
		jobTop.getConfiguration().set(PRIMARY_KEY, "origin,departureperf");
		...
		String query = "UPDATE " + KEYSPACE + "." + OUTPUT_COLUMN_FAMILY + " SET destination = ? ";
...
//		CREATE TABLE group2.question2( origin TEXT, destination TEXT, departureperf float, PRIMARY KEY ((origin),departureperf) );
	...
	public static class CountAirportCarrierMap extends Mapper<Object, Text, Text, FloatWritable> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			// line is rejected
if (fields.length > 11) {
			if (!fields[8].isEmpty() && !fields[7].isEmpty() && !fields[11].isEmpty())

			{
				context.write(
						new Text(new StringBuilder(fields[7]).append(":").append(fields[8]).toString()),
						new FloatWritable(Float.parseFloat(fields[11])));
			} else {
...
{
	if (!fields[8].isEmpty() && !fields[7].isEmpty() )

	{
		context.write(
				new Text(new StringBuilder(fields[7]).append(":").append(fields[8]).toString()),
				new FloatWritable(Float.parseFloat("0.00")));
	} 
	...
	public static class TopAirportAirportMap extends Mapper<Text, Text, NullWritable, TextArrayWritable> {
		...
		private HashMap<String, TreeSet<Pair<Float, String>>> countByAirportAirportDepMap = new HashMap<String, TreeSet<Pair<Float, String>>>();
		...
		public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] AirPortCarrierCode = key.toString().split(":");// decouple
																	// the key
			String AirportOriginCode = AirPortCarrierCode[0];
			String AirportDestinationCode = AirPortCarrierCode[1];
			if (!countByAirportAirportDepMap.containsKey(AirportOriginCode)) {
			
				countByAirportAirportDepMap.put(AirportOriginCode, new TreeSet<Pair<Float, String>>());
			}
			//System.out.println(AirportOriginCode + ">" + AirportDestinationCode);
			TreeSet<Pair<Float, String>> AirportTreeSet = countByAirportAirportDepMap.get(AirportOriginCode);
			AirportTreeSet.add(new Pair<Float, String>(count, AirportDestinationCode));
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			while (i.hasNext()) {
				...
				TreeSet<Pair<Float, String>> AirportCount = countByAirportAirportDepMap.get(currentKey);
	...
	public static class TopAirportAirportReduceToCassandra
...
public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context)
	...
	for (TextArrayWritable val : values) {
		...
		if (!countByAirportAirportDepMap.containsKey(OriginAirportCode)) {

			countByAirportAirportDepMap.put(OriginAirportCode, new TreeSet<Pair<Float, String>>());
		}
		// System.out.println(AirPortCode + ">" + CarrierCode);
		TreeSet<Pair<Float, String>> AirportTreeSet = countByAirportAirportDepMap.get(OriginAirportCode);
		AirportTreeSet.add(new Pair<Float, String>(count, DestinationAirportCode));
		if (AirportTreeSet.size() > this.N) {
			AirportTreeSet.remove(AirportTreeSet.last());
		}
	...
	while (i.hasNext()) {
		Object currentKey = i.next();
		Compositekeys = new LinkedHashMap<String, ByteBuffer>();
		Compositekeys.put("origin", ByteBufferUtil.bytes(currentKey.toString()));

		TreeSet<Pair<Float, String>> AirportCount = countByAirportAirportDepMap.get(currentKey);
		for (Pair<Float, String> item : AirportCount) {
			Compositekeys.put("departureperf", ByteBufferUtil.bytes(item.first));
			...
			context.write(Compositekeys,
					getBindVariables(currentKey.toString(), item.second , item.first.toString() ));
		}
	} 

}

private List<ByteBuffer> getBindVariables(String Origin, String Destination, String perf) {
	List<ByteBuffer> variables = new ArrayList<ByteBuffer>();
	// variables.add(Compositekeys.get("row_id1"));
	// variables.add(Compositekeys.get("row_id2"));
	// variables.add(ByteBufferUtil.bytes(airport));
	 variables.add(ByteBufferUtil.bytes(Destination));
	...
	public static class TopAirportAirportReduce extends Reducer<NullWritable, TextArrayWritable, Text, FloatWritable> {
		Integer N;
		// private TreeSet<Pair<Float, String>> countToAirportMap = new
		// TreeSet<Pair<Float, String>>();
