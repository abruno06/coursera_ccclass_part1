 ...
 */
public final class readtopairportinairport {
	...
	public static void main(String[] args) {
		...
		JavaRDD<AirportToAirport> flyRDDSRQ = cassandraRDD.where("origin=?", "SRQ")
				.map(new Function<CassandraRow, AirportToAirport>() {

					@Override
					public AirportToAirport call(CassandraRow row) throws Exception {
						AirportToAirport fly = new AirportToAirport();
		...
		for (AirportToAirport result : flyRDDSRQ.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.departureperf));
		}

		/// CMH reading
		System.out.println("--------------- CMH --------------");
		
		JavaRDD<AirportToAirport> flyRDDCMH = cassandraRDD.where("origin=?", "CMH")
				.map(new Function<CassandraRow, AirportToAirport>() {
		...
		for (AirportToAirport result : flyRDDCMH.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.departureperf));
		}
		...
		JavaRDD<AirportToAirport> flyRDDJFK = cassandraRDD.where("origin=?", "JFK")
				.map(new Function<CassandraRow, AirportToAirport>() {
		...
		for (AirportToAirport result : flyRDDJFK.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.departureperf));
		}
		...
		JavaRDD<AirportToAirport> flyRDDSEA = cassandraRDD.where("origin=?", "SEA")
				.map(new Function<CassandraRow, AirportToAirport>() {
		...
		for (AirportToAirport result : flyRDDSEA.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.departureperf));
		}
		...
		JavaRDD<AirportToAirport> flyRDDBOS = cassandraRDD.where("origin=?", "BOS")
				.map(new Function<CassandraRow, AirportToAirport>() {
		...
		for (AirportToAirport result : flyRDDBOS.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.departureperf));
		}
