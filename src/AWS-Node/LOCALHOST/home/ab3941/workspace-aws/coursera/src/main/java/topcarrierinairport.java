 ...
 */ 
public final class topcarrierinairport {
	...
	public static void main(String[] args) {
		...
		try (Session session = connector.openSession()) {
			...
			session.execute(
					"CREATE KEYSPACE topcarrierinairport WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 3}");
			...
			session.execute("CREATE TABLE topcarrierinairport.table1( airport TEXT, carrier TEXT, departureperf float, PRIMARY KEY ((airport),carrier) )");
		...
		JavaDStream<String> airport = fly.flatMap(new FlatMapFunction<String, String>() {
			...
			public Iterable<String> call(String x) {
				...
				// {
				return Lists.newArrayList(fields[4] + ":" + fields[2] + "," + fields[9]);
				// }
				// else
				// {
				// return
...
//				});
		...
		results.foreachRDD(new Function<JavaRDD<AirportCarrier>, Void>() {
			...
			public Void call(JavaRDD<AirportCarrier> rdd) {
				 ...
				 javaFunctions(rdd).writerBuilder("topcarrierinairport","table1",mapToRow(AirportCarrier.class) ).saveToCassandra();
	...
	public static class AirportCarrier implements Serializable {
