...
public final class readorigindestinationdelay {
	...
	public static void main(String[] args) {
		...
		System.out.println("--------------- LGA → BOS --------------");
		JavaRDD<AirportToAirportArrival> flyRDD1 = cassandraRDD.where("origin=? AND destination=?", "LGA" , "BOS")
				.map(new Function<CassandraRow, AirportToAirportArrival>() {

					@Override
					public AirportToAirportArrival call(CassandraRow row) throws Exception {
						AirportToAirportArrival fly = new AirportToAirportArrival();
						fly.setOrigin(row.getString("origin"));
						fly.setDestination(row.getString("destination"));
						fly.setMeandelay(row.getFloat("meandelay"));
						return fly;
					}
				});

		for (AirportToAirportArrival result : flyRDD1.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.meandelay));
		}

		/// BOS → LGA
		System.out.println("--------------- BOS → LGA --------------");
		
		JavaRDD<AirportToAirportArrival> flyRDD2 = cassandraRDD.where("origin=? AND destination=?", "BOS" , "LGA")
				.map(new Function<CassandraRow, AirportToAirportArrival>() {
		...
		for (AirportToAirportArrival result : flyRDD2.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.meandelay));
		}
		...
		System.out.println("--------------- OKC → DFW --------------");
		
		JavaRDD<AirportToAirportArrival> flyRDD3 = cassandraRDD.where("origin=? AND destination=?", "OKC" , "DFW")
				.map(new Function<CassandraRow, AirportToAirportArrival>() {
		...
		for (AirportToAirportArrival result : flyRDD3.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.meandelay));
		}

		
		//MSP → ATL
		System.out.println("--------------- MSP → ATL --------------");
		
		JavaRDD<AirportToAirportArrival> flyRDD4 = cassandraRDD.where("origin=? AND destination=?", "MSP" , "ATL")
				.map(new Function<CassandraRow, AirportToAirportArrival>() {
		...
		for (AirportToAirportArrival result : flyRDD4.top(10)) {
			System.out.println(String.format("%s - %s - %f", result.origin, result.destination, result.meandelay));
		}
	...
	public static class AirportToAirport implements Serializable,Comparable {
		// ( airport TEXT, carrier TEXT, departureperf float, PRIMARY KEY
		// ((airport),departureperf) )");
		...
		@Override
		public int compareTo(Object obj) {
			AirportToAirport o = (AirportToAirport) obj;
			int cmp = o == null ? 1 : (this.departureperf).compareTo(o.departureperf);
			return cmp == 0 ? (this.destination).compareTo(o.destination) : cmp;
		}

	}	
	
	
	public static class AirportToAirportArrival implements Serializable,Comparable {
		// ( airport TEXT, carrier TEXT, departureperf float, PRIMARY KEY
		// ((airport),departureperf) )");
		private String origin;
		private String destination;
		...
		@Override
		public int compareTo(Object obj) {
			AirportToAirportArrival o = (AirportToAirportArrival) obj;
			int cmp = o == null ? 1 : (this.meandelay).compareTo(o.meandelay);
			return cmp == 0 ? (this.destination).compareTo(o.destination) : cmp;
		}
